...
const NewsPage = () => {
	...
	const filterSortList = list.sort(
		(a: NewsItem, b: NewsItem) => new Date(b.createdDate).getTime() - new Date(a.createdDate).getTime()
	);
						...
						{newsList?.slice(1, 5).map(
							(
								...
								>
									...
									<div className="text-base font-semibold mt-2 two-line-truncate">
										{_.mainTitle}
									</div>
					...
					{filterSortList?.slice(0, visibleCount).map(
						(
							...
							>
								...
								<div className="text-base font-semibold group-hover:text-gradient  two-line-truncate">
									{news.mainTitle}
								</div>
							</Link>
						)
