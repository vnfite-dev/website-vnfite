            ...
            {Array.from({ length: 5 }).map((_, index) => (
                <div className="flex items-center cursor-pointer" key={index} onClick={() => setColorClick(index)}>
                    ...
                    >
                        ...
                        <div className={`absolute ${colorClick === index ? 'text-gradient' : 'text-[#B3B3B3]'} text-lg lg:text-2xl font-semibold top-9 -left-3 lg:-left-5`}>{index + 2020}</div>
                    </div>
                    <div className={`w-10 lg:w-20 h-1 ${colorClick === index ? 'bg-grad' : 'bg-[#B3B3B3]'}`}></div>
                </div>
            ))
            }
        </div >
