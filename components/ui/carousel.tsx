...
const Carousel = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & CarouselProps>(
	(
		{ onIndexChange, orientation = "horizontal", opts, setApi, plugins, className, children, ...props },
		ref
	) => {
		...
		const onSelect = React.useCallback(
			(api: CarouselApi) => {
				...
				if (onIndexChange) {
					onIndexChange(currentIndex); // Gọi callback với giá trị index mới
				}
			},
			[onIndexChange]
		);
		...
		return (
			<div ref={ref} className={`flex justify-center ${props.className}`}>
				{Array.from({ length: numberOfSlides }, (_, i) => (
					<Button
						key={i}
						className={`mx-1 h-2 rounded-full p-0 transition-all duration-300 ease-in-out ${
							i === currentSlide
